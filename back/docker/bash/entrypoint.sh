...
import psycopg2
try:
    print("Trying to connect to database '$SQL_DATABASE' on host '$SQL_HOST'..")
    conn = psycopg2.connect(dbname="$SQL_DATABASE", user="$SQL_USER", password="$SQL_PASSWORD", host="$SQL_HOST")
